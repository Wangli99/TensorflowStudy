...
with tf.Session() as sess:
    ...
    for step in range(1000):
        ...
        if step % 50 == 0:
            test = sess.run(merged, feed_dict={xs:mnist.test.images, ys:mnist.test.labels})
            train = sess.run(merged, feed_dict={xs:batch_xs, ys:batch_ys})
            final_accuracy = sess.run(accuracy, feed_dict={xs:mnist.test.images, ys:mnist.test.labels})
            testWriter.add_summary(test, step)
            trainWriter.add_summary(train, step)
